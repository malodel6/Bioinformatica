...
str(P01923)
nombre_ejercicio3 <- P01923[['metadata']][['sequence']]
nombre_ejercicio3
```
...
ejercicio_4 <- fromJSON(paste0(API, 'taxonomy/protein'))[['taxa']][['uniprot']]

resultado <- taxonomy - ejercicio_4
resultado
```
1387422 taxones sin proteínas asignadas a ellos. El motivo por el que hay tantos taxones sin proteínas asignadas probablemente sea porque en las proteínas no revisadas se han dejado de añadir las de taxones muy parecidos a otros que tienen proteínas en su base de datos, ya que entonces se asumirán que estas proteínas tan similares serán la de ese taxón hermano también.

```{r}
busqueda1 <- fromJSON(paste0(API, 'protein'))[['proteins']][['uniprot']]
busqueda2 <- fromJSON(paste0(API, 'protein/structure'))[['proteins']][['uniprot']]
proporcion <- busqueda2 / busqueda1 * 100
proporcion
```
0.03357599% de las proteínas tienen estructura terciaria.
:::
...
flatten(vertebrados[['results']][['metadata']])
```
...
```{r}
ejercicio_6 <- fromJSON(paste0(API, 'protein/taxonomy/uniprot/8493/entry/pfam/PF01530'))
ejercicio_6[[1]]
```
Hay 15 proteínas, no revisadas.
...
# Si no existe, la creamos:
   ...
   # Aquí es donde guardamos una variable de R de una sesión en un archivo Rds:
...
equinodermata[equinodermata$accession %in% SoloEquinodermos,
              c('accession', 'name')]
```
