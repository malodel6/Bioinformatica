...
Sería conveniente, ya que así podemos hacer diversos análisis refiriéndonos al mismo archivo, ya además si ocupan mucho mejor que estén en una carpeta separada para que si alguien quiere leer solo los resultados, no hace falta que se descargue los archivos de datos.
...
zless es el programa navegador de archivos fastq. Se ejecuta en el terminal como zless y el nombre del archivo. Para salir pulsar la q.

wc (word count) -l nombre del archivo (antes de leer el archivo fastq se tendrá que extraer con gunzip -c nombre del archivo. -c para que no lo descomprima, solo se meta en la carpeta sin descomprimir). Al hacer una tubería (|) se puede hacer el comando wc entonces, dándote el número sin necesitar descomprimir el archivo. gunzip -c DRR025089_1.fastq.gz | less -S

grep (regular expression) Se puede pedir a un archivo que muestre las líneas en las que coincida la expresión regular que se le da (por ejemplo, una palabra)
Para saber el nº de secuencias en un archivo fasta, se haría grep ">" (ya que los fasta tienen las secuencias separadas por el símbolo >) y el nombre del archivo. Si se le añade un ^ delante de la expresión regular, indica que busca esa expresión solo al inicio de la línea.